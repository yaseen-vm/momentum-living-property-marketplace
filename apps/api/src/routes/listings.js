...
listingRoutes.get("/", async (c) => {
    const { type, location, min_price, max_price, min_size, amenities, sort, limit, offset } = c.req.query();
    ...
    const countResult = await c.env.DB.prepare(`SELECT COUNT(*) as total FROM listings l WHERE ${where}`)
        .bind(...params)
        .first();
    const rows = await c.env.DB.prepare(`SELECT l.id, l.type, l.title, l.price, l.currency, l.location_text,
...
listingRoutes.get("/:id", async (c) => {
    ...
    const listing = await c.env.DB.prepare(`SELECT l.*, u.name as vendor_name
     FROM listings l
    ...
    const photos = await c.env.DB.prepare("SELECT r2_key, display_order FROM listing_photos WHERE listing_id = ? ORDER BY display_order ASC")
        .bind(id)
        .all();
