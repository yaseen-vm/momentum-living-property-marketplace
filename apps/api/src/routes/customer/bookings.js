...
customerBookingRoutes.post("/", requireAuth(["customer"]), async (c) => {
    ...
    const listing = await c.env.DB.prepare(`SELECT l.id, l.title, l.type, l.location_text, l.status,
    ...
    const existing = await c.env.DB.prepare("SELECT id FROM bookings WHERE customer_id = ? AND listing_id = ?")
    ...
    const now = Date.now();
    await c.env.DB.prepare("INSERT INTO bookings (id, customer_id, listing_id, status, customer_name, customer_email, customer_alt_mobile, created_at, updated_at) VALUES (?, ?, ?, 'pending', ?, ?, ?, ?, ?)")
        .bind(bookingId, payload.sub, body.listing_id, body.name, body.email, body.alt_mobile ?? null, now, now)
        .run();
    c.executionCtx.waitUntil((async () => {
        await c.env.DB.prepare("INSERT INTO admin_notifications (id, type, payload, created_at) VALUES (?, 'new_booking', ?, ?)")
            .bind(crypto.randomUUID(), JSON.stringify({ booking_id: bookingId, listing_id: body.listing_id }), now)
            .run();
...
customerBookingRoutes.get("/", requireAuth(["customer"]), async (c) => {
    const payload = c.get("jwtPayload");
    const rows = await c.env.DB.prepare(`SELECT b.id, b.status, b.created_at,
