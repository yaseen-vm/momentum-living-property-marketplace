...
adminReportRoutes.get("/", requireAuth(["admin"]), async (c) => {
    ...
    const results = await c.env.DB.batch([
        c.env.DB.prepare(`SELECT status, COUNT(*) as cnt FROM listings
         WHERE created_at >= ? AND created_at <= ? GROUP BY status`).bind(fromMs, toMs),
        c.env.DB.prepare(`SELECT status, COUNT(*) as cnt FROM vendor_profiles
         WHERE created_at >= ? AND created_at <= ? GROUP BY status`).bind(fromMs, toMs),
        c.env.DB.prepare(`SELECT COUNT(*) as total,
                SUM(CASE WHEN mobile_verified_at IS NOT NULL THEN 1 ELSE 0 END) as verified
         FROM users WHERE role = 'customer' AND created_at >= ? AND created_at <= ?`).bind(fromMs, toMs),
        c.env.DB.prepare(`SELECT status, COUNT(*) as cnt FROM bookings
         WHERE created_at >= ? AND created_at <= ? GROUP BY status`).bind(fromMs, toMs),
        c.env.DB.prepare(`SELECT type, COUNT(*) as cnt FROM listings
         WHERE status = 'approved' AND created_at >= ? AND created_at <= ? GROUP BY type`).bind(fromMs, toMs),
    ]);
    const [listingStats, vendorStats, customerStats, bookingStats, listingByType] = results;
    function toMap(rows) {
        return Object.fromEntries(rows.map((r) => [r.status, r.cnt]));
    }
    const lMap = toMap(listingStats.results);
    const vMap = toMap(vendorStats.results);
    const bMap = toMap(bookingStats.results);
    const tMap = toMap(listingByType.results);
    const cRow = (customerStats.results[0] ?? { total: 0, verified: 0 });
